...
const data = [
  ...
  },
  {
    ...
    title: "Angst",
    description:
      "Die Rücken nass, die Hände klamm Alle haben Angst Schwarzer Mann",
    content:
    ...
    <main onMouseMove={mouseTracker} className="h-full w-full py-16">
      <div className="max-w-screen-lg mx-auto p-4 md:p-0">
        <div className="flex flex-row justify-center items-center gap-1 w-full mb-8">
          <h1 className="text-4xl lg:text-6xl font-PlayfairDisplay">
            Loud Thoughts
          </h1>
          <hr className="w-full h-0.5 mx-auto border-0 rounded md:my-10 bg-zinc-300" />
          <Link
            href="/work"
            className="text-2xl lg:text-4xl font-PlayfairDisplay text-end"
          >
            See All
          </Link>
        </div>
        {featuredPosts.map((item: PostTypes) => {
          return (
            <Vignette mousePosition={mousePosition} item={item} key={item.id} />
          );
        })}
      </div>
    </main>
